...
int lzlib4::compress_block(lzlib4_flush_mode flush_mode) {
    ...
    while (true) {
        ...
        if (to_compress) {
            ...
            if (compressed) {
                ...
                uint32_t crc = crc32((char *)strm.state.compress_in_buffer, strm.state.compress_in_index);
                // Add block header
...
int lzlib4::decompress_block(int64_t seek_to_pos, bool check_crc) {
    ...
    // Check if input buffer doesn't contains the entire block
...
uint32_t lzlib4::crc32(char *buf, size_t len) {
